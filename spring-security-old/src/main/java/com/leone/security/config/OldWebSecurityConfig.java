...
//                .and().csrf().disable();
        ...
        http.authorizeRequests()
                .antMatchers("/login-page.html", "/authentication/form", "/error/4xx.html").permitAll()
                .anyRequest().authenticated()
                .and()
                .formLogin().loginPage("/login-page.html").loginProcessingUrl("/authentication/form")
                .successForwardUrl("/index").failureUrl("/error/4xx.html")
                .and()
                .logout().logoutUrl("/logout").permitAll().logoutSuccessUrl("/logout-page.html")
                .and()
                .csrf().disable();
    }
}